"use strict";
// Ensure x is a valid audio or video element
var x = document.querySelector('video'); // or 'audio', depending on your use case
...
document.addEventListener("anura-login-completed", async () => {
    ...
    if (!requiredDirectories) {
        await anura.settings.set("requiredDirectories", (requiredDirectories = ["apps", "libs", "init", "opt"]));
    }
    ...
    if (anura.settings.get("mute")) {
        anura.notifications.add({
            title: "RainbowOS Error",
            description: "Sorry, RainbowOS is unable to mute due to an error. Please contact customer support for more information.",
    });
}
if (anura.settings.get("lol_boot")) {
    function playAudio() {
        if (x) { // Check if x is defined
            x.setAttribute('autoplay', ''); // or x.autoplay = true;
            x.play(); // Make sure to call play() to start playback
        } else {
            console.error('Audio element not found');
        }
    }

    // Call the function to apply the autoplay
    playAudio();
}
